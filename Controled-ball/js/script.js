...
function createWorld() {
    ...
    light.position.set(-60, 60, 1);
    camera.position.set(25, 40, 50);
    camera.lookAt(scene.position);
    light2.position.set(60, 60, 1);
    light3.position.set(0, -60, 1);
    //camera.add(light);
    camera.add(light2);
    camera.add(light3);
    scene.add(camera);
    ...
    cube = new THREE.Mesh(new THREE.SphereGeometry(10, 70,70), new THREE.MeshPhongMaterial({color: 0x0000FF }));

    scene.add(cube);

    var edgeGeometry = new THREE.EdgesGeometry(cube.geometry);
    cube.add(new THREE.LineSegments(edgeGeometry, new THREE.LineBasicMaterial({color: 0x000000})));
